...
module OpenapiClient
  class SetDeviceNameRequest
    # The new name for the device.  This can be provided as either the fully qualified domain name for the device (e.g. \"nodename.your-domain.ts.net\") or just the base name (e.g. \"nodename\").  If `name` is unset or provided empty, the device's name is reset to be generated from its OS hostname. 
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SetDeviceNameRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SetDeviceNameRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
