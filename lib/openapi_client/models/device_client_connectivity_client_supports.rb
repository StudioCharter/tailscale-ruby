...
module OpenapiClient
  ...
  class DeviceClientConnectivityClientSupports
    # 'true' if your router can route connections from endpoints on your LAN back to your LAN using those endpoints’ globally-mapped IPv4 addresses/ports. 
    attr_accessor :hair_pinning

    # 'true' if the device OS supports IPv6, regardless of whether IPv6 internet connectivity is available. 
    attr_accessor :ipv6
    ...
    # 'true' if UDP traffic is enabled on the current network; if 'false', Tailscale may be unable to make direct connections, and will rely on our DERP servers. 
    attr_accessor :udp
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::DeviceClientConnectivityClientSupports` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::DeviceClientConnectivityClientSupports`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'hair_pinning')
        self.hair_pinning = attributes[:'hair_pinning']
      end
      ...
      if attributes.key?(:'pmp')
        self.pmp = attributes[:'pmp']
      end

      if attributes.key?(:'udp')
        self.udp = attributes[:'udp']
      end

      if attributes.key?(:'upnp')
        self.upnp = attributes[:'upnp']
      end
    end
    ...
    def hash
      [hair_pinning, ipv6, pcp, pmp, udp, upnp].hash
    end
