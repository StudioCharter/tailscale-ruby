...
module OpenapiClient
  class SetCustomDevicePostureAttributesRequest
    attr_accessor :value

    # An optional expiry time for a given posture attribute. If set, Tailscale will automatically remove the attribute within a few minutes after the specified time. 
    attr_accessor :expiry

    # An optional comment indicating a reason why an attribute is set, which will be added to the audit log. 
    ...
    def self.openapi_types
      {
        :'value' => :'SetCustomDevicePostureAttributesRequestValue',
        :'expiry' => :'Time',
        :'comment' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SetCustomDevicePostureAttributesRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SetCustomDevicePostureAttributesRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@comment.nil? && @comment.to_s.length > 200
        invalid_properties.push('invalid value for "comment", the character length must be smaller than or equal to 200.')
      end
    ...
    def valid?
      ...
      return false if !@comment.nil? && @comment.to_s.length > 200
      true
    end
    ...
    def comment=(comment)
      ...
      if comment.to_s.length > 200
        fail ArgumentError, 'invalid value for "comment", the character length must be smaller than or equal to 200.'
      end
