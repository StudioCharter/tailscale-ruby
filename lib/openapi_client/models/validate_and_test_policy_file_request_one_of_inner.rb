...
module OpenapiClient
  class ValidateAndTestPolicyFileRequestOneOfInner
    # Specifies the user identity to test, which can be  a [user's email address](https://tailscale.com/kb/1337/acl-syntax#reference-users), a [group](https://tailscale.com/kb/1337/acl-syntax#groups), a [tag](https://tailscale.com/kb/1068/acl-tags), or a [host](https://tailscale.com/kb/1337/acl-syntax#hosts) that maps to an IP address. The test case runs from the perspective of a device authenticated with the provided identity. 
    attr_accessor :src

    # Specifies the [device posture attributes](https://tailscale.com/kb/1337/acl-syntax#proto-1) as key-value pairs to use when evaluating posture conditions in access rules. You only need to use this field if the access rules contain  [device posture conditions](https://tailscale.com/kb/1288/device-posture#device-posture-conditions). 
    attr_accessor :src_posture_attrs

    # Specifies the IP protocol for `accept` and `deny` rules, similar to the `proto` field in [ACL rules](https://tailscale.com/kb/1337/acl-syntax#acls). When omitted, the test checks for either TCP or UDP access. 
    attr_accessor :proto

    # Specifies destinations to accept. Each destination in the list is of the form `host:port` where `port` is a single numeric port and `host` is in the format described in the [acl syntax](https://tailscale.com/kb/1337/acl-syntax#accept-and-deny-destinations) documentation.  Sources in `src` and `destinations` must refer to specific entities and do not support `*` wildcards. For example, an `accept` destination cannot be `tags:*`. 
    attr_accessor :accept

    # Specifies destinations to deny. Each destination in the list is of the form `host:port` where `port` is a single numeric port and `host` is in the format described in the [acl syntax](https://tailscale.com/kb/1337/acl-syntax#accept-and-deny-destinations) documentation.  Sources in `src` and `destinations` must refer to specific entities and do not support `*` wildcards. For example, a `deny` destination cannot be `tags:*`. 
    attr_accessor :deny
    ...
    def self.attribute_map
      {
        :'src' => :'src',
        :'src_posture_attrs' => :'srcPostureAttrs',
        :'proto' => :'proto',
        :'accept' => :'accept',
        :'deny' => :'deny'
      }
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ValidateAndTestPolicyFileRequestOneOfInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ValidateAndTestPolicyFileRequestOneOfInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'src_posture_attrs')
        if (value = attributes[:'src_posture_attrs']).is_a?(Hash)
          self.src_posture_attrs = value
        end
      end
      ...
      if attributes.key?(:'accept')
        if (value = attributes[:'accept']).is_a?(Array)
          self.accept = value
        end
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          src == o.src &&
          src_posture_attrs == o.src_posture_attrs &&
          proto == o.proto &&
          accept == o.accept &&
          deny == o.deny
    end
    ...
    def hash
      [src, src_posture_attrs, proto, accept, deny].hash
    end
