  ...
  class DnsSearchPaths
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::DnsSearchPaths` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::DnsSearchPaths`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'search_paths')
        if (value = attributes[:'search_paths']).is_a?(Array)
          self.search_paths = value
        end
      else
        self.search_paths = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @search_paths.nil?
        invalid_properties.push('invalid value for "search_paths", search_paths cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @search_paths.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] search_paths Value to be assigned
    def search_paths=(search_paths)
      if search_paths.nil?
        fail ArgumentError, 'search_paths cannot be nil'
      end
