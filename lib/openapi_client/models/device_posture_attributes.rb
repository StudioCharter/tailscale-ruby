...
module OpenapiClient
  class DevicePostureAttributes
    # Contains all the posture attributes assigned to a node. Attribute values can be strings, numbers or booleans. 
    ...
    def self.openapi_types
      {
        :'attributes' => :'Hash<String, ValidateAndTestPolicyFileRequestOneOfInnerSrcPostureAttrsValue>',
        :'expiries' => :'Hash<String, Time>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::DevicePostureAttributes` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::DevicePostureAttributes`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'expiries')
        if (value = attributes[:'expiries']).is_a?(Hash)
          self.expiries = value
        end
      end
    end
