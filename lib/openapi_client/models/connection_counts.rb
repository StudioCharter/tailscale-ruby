  ...
  class ConnectionCounts
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ConnectionCounts` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ConnectionCounts`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'tx_pkts')
        self.tx_pkts = attributes[:'tx_pkts']
      end

      if attributes.key?(:'tx_bytes')
        self.tx_bytes = attributes[:'tx_bytes']
      end

      if attributes.key?(:'rx_pkts')
        self.rx_pkts = attributes[:'rx_pkts']
      end

      if attributes.key?(:'rx_bytes')
    ...
    def valid?
      ...
      proto_validator = EnumAttributeValidator.new('String', ["ah", "dccp", "egp", "esp", "gre", "icmp", "igmp", "igp", "ipv4", "ipv6-icmp", "sctp", "tcp", "udp"])
    ...
    def proto=(proto)
      validator = EnumAttributeValidator.new('String', ["ah", "dccp", "egp", "esp", "gre", "icmp", "igmp", "igp", "ipv4", "ipv6-icmp", "sctp", "tcp", "udp"])
    ...
    def hash
      [proto, src, dst, tx_pkts, tx_bytes, rx_pkts, rx_bytes].hash
    end
