...
module OpenapiClient
  ...
  class ConfigurationAuditLogActor
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ConfigurationAuditLogActor` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ConfigurationAuditLogActor`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["USER", "NODE", "AUTOMATED_WORKER", "OAUTH_CLIENT", "SCIM", "MULLVAD", "LOGSTREAM", "SECRET_SCANNER"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["USER", "NODE", "AUTOMATED_WORKER", "OAUTH_CLIENT", "SCIM", "MULLVAD", "LOGSTREAM", "SECRET_SCANNER"])
    ...
    def hash
      [id, type, login_name, display_name, tags].hash
    end
